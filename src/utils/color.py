...
class Color:
    ...
    def pulsing(self):
        if self.pulsing_direction > 0:
            self.red   = self.red + self.pulsing_direction if self.red + self.pulsing_direction < 255 else 255
            self.green = self.green + self.pulsing_direction if self.green + self.pulsing_direction < 255 else 255
            self.blue  = self.blue + self.pulsing_direction if self.blue + self.pulsing_direction < 255 else 255
            if self.red == 255 and self.green == 255 and self.blue == 255:
                self.pulsing_direction *= -1
        if self.pulsing_direction < 0:
            self.red   = self.red + self.pulsing_direction if self.red + self.pulsing_direction > 0 else 0
            self.green = self.green + self.pulsing_direction if self.green + self.pulsing_direction > 0 else 0
            self.blue  = self.blue + self.pulsing_direction if self.blue + self.pulsing_direction > 0 else 0
